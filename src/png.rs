...
impl Png {
    ...
    pub const CHUNK_TYPE_BYTES: u32 = 4;
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        [&self.header, chunk_bytes.as_slice()].concat()
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self> {
        ...
        while reader.buffer().len() > 0 {
            ...
            let length_left: usize = (length + Self::CHUNK_TYPE_BYTES + Self::CRC_BYTES) as usize;

            let mut rest_of_chunks: vec::Vec<u8> = vec![0; length_left];
            reader.read_exact(&mut rest_of_chunks)?;

            let chunk_bytes = [&length_buffer, rest_of_chunks.as_slice()].concat();
...
impl fmt::Display for Png {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Png bytes: {:?}", self.as_bytes())
    }
}
...
mod tests {
    ...
    fn test_from_chunks() {
        let chunks = testing_chunks();
        let png = Png::from_chunks(chunks.clone());
